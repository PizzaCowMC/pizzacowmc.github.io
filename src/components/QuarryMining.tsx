...
export const QuarryMining: React.FC<QuarryMiningProps> = ({
  ...
  // Calculate mining speed multiplier (with haste drink 2x multiplier)
  const hasteMultiplier = hasteRemainingSeconds > 0 ? 2.0 : 1.0;
  ...
  const completeMine = useCallback(() => {
    ...
    const textStr = amount > 1 ? `+${amount} ${activeBlock.nameZh} (幸運!)` : `+${amount} ${activeBlock.nameZh}`;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4 pb-3 border-b-2 border-dashed border-zinc-700">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1">
            當前開採地層：<strong className="text-amber-300">{activeLayer.nameZh}</strong> • 累計總挖掘量：
            <span className="text-amber-400 font-mono font-bold ml-1">{totalBlocksMined.toLocaleString()}</span> 格
          </p>
        </div>

        <div className="flex items-center gap-2 text-xs font-mono bg-zinc-900 px-3 py-1.5 rounded border-2 border-black">
          <Layers className="w-4 h-4 text-amber-400" />
          <span className="text-zinc-400">當前地層挖掘量:</span>
          <span className="text-amber-300 font-bold">{currentLayerMined.toLocaleString()} 格</span>
        </div>
      </div>
        ...
        <div className={`mb-4 p-3 rounded-lg border-2 border-black flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-colors ${
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${
                  ...
                  {isNextLayerUnlocked ? '✅ 已解鎖可前往' : `需在此層挖滿 50,000 格 (${currentLayerProgressPct}%)`}
                </span>
              </div>
              <p className="text-[11px] text-zinc-400 mt-0.5">
                {isNextLayerUnlocked
                  ? `太棒了！已在「${activeLayer.nameZh}」挖滿 50,000 格，點擊下方標籤即可切換至下一層！`
                  : `還需挖掘 ${(unlockThreshold - currentLayerMined).toLocaleString()} 格方塊即可解鎖「${nextLayer.nameZh}」！`}
              </p>
            </div>
          </div>
          ...
          <div className="w-full sm:w-56 shrink-0">
            <div className="flex justify-between text-[10px] font-mono text-zinc-400 mb-1">
              <span>5萬格進度</span>
              <span className="text-amber-300 font-bold">{currentLayerMined.toLocaleString()} / 50,000</span>
            </div>
      ...
      <div className="mb-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
          {STRATA_LAYERS.map((layer, idx) => {
            ...
            const prevLayer = idx > 0 ? STRATA_LAYERS[idx - 1] : null;
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-center">
        ...
        <div className="md:col-span-7 bg-zinc-950 border-4 border-black p-6 rounded-lg text-center relative overflow-hidden shadow-[inset_0_0_20px_rgba(0,0,0,0.9)] flex flex-col items-center justify-center min-h-[270px]">
          ...
          <div className="mb-2">
            <span className="text-xs px-2 py-0.5 rounded bg-zinc-900 border border-zinc-700 text-amber-300 font-mono">
              {activeLayer.icon} {activeLayer.nameZh} • Hardness: {activeBlock.hardness}s
            </span>
            <div className="text-lg font-black text-amber-300 drop-shadow-[1px_1px_0_#000] mt-1">
              {activeBlock.nameZh} ({activeBlock.nameEn})
            </div>
        ...
        <div className="md:col-span-5 bg-zinc-900 border-3 border-black p-4 rounded-lg flex flex-col justify-between h-full shadow-[inset_2px_2px_0_#3f3f46]">
          <div>
            <div className="flex items-center justify-between pb-2 border-b border-zinc-800 mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-black text-white text-sm">裝備鎬具狀態</span>
              </div>
              <button
                onClick={onOpenShopToPickaxes}
                className="text-[11px] px-2 py-1 bg-amber-700 hover:bg-amber-600 text-amber-100 font-bold rounded border border-black transition-colors flex items-center gap-1 shadow-[inset_1px_1px_0_#fde047]"
              >
            ...
            <div className="bg-zinc-950 p-3 border-2 border-black rounded mb-3">
              ...
              <div>
                <div className="flex justify-between text-[11px] font-mono mb-1">
                  <span className="text-zinc-400">鎬子耐久度</span>
                  {currentPickaxe.tier === 0 ? (
                    <span className="text-emerald-400 font-bold">無限耐久</span>
                  ) : (
            ...
            <div className="space-y-1.5 text-xs">
              ...
              <div className="flex items-center justify-between p-1.5 bg-zinc-950/70 border border-zinc-800 rounded">
                ...
                <span className="font-mono font-bold text-blue-400">
                  Lv. {pickaxeState.unbreakingLevel} ({pickaxeState.unbreakingLevel * 10}% 減損)
                </span>
              </div>
              <div className="flex items-center justify-between p-1.5 bg-zinc-950/70 border border-zinc-800 rounded">
                <span className="flex items-center gap-1.5 text-zinc-300">
                ...
                <span className="font-mono font-bold text-emerald-400">
                  Lv. {pickaxeState.fortuneLevel} ({pickaxeState.fortuneLevel * 18}% 機率翻倍)
                </span>
              </div>
          ...
          <div className="text-[10px] text-zinc-500 mt-3 border-t border-zinc-800 pt-2 text-center">
            💡 累積挖掘 50,000 格可解鎖下一層深邃地層，獲得更稀有寶石神礦！
          </div>
        </div>
      </div>
    </section>
