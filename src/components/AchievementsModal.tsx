...
export const AchievementsModal: React.FC<AchievementsModalProps> = ({
  ...
  const unclaimedAchievements = useMemo(() => {
    return achievements.filter(a => a?.unlocked && a.coinReward > 0 && !a.rewardClaimed);
  }, [achievements]);

  const totalUnclaimedCoins = useMemo(() => {
    return unclaimedAchievements.reduce((sum, a) => sum + (a.coinReward || 0), 0);
  ...
  const filteredList = useMemo(() => {
    ...
    return achievements.filter(a => {
      ...
      if (selectedCat === 'unclaimed') {
        if (!a.unlocked || !a.coinReward || a.rewardClaimed) return false;
      ...
      if (q) {
        const nameZhMatch = a.nameZh ? a.nameZh.toLowerCase().includes(q) : false;
        const nameEnMatch = a.nameEn ? a.nameEn.toLowerCase().includes(q) : false;
        const descZhMatch = a.descZh ? a.descZh.toLowerCase().includes(q) : false;
        const descEnMatch = a.descEn ? a.descEn.toLowerCase().includes(q) : false;
        return nameZhMatch || nameEnMatch || descZhMatch || descEnMatch;
      }
      return true;
    });
  }, [isOpen, achievements, selectedCat, searchQuery]);

  const totalPages = Math.max(1, Math.ceil(filteredList.length / PAGE_SIZE));
  const safePage = Math.min(currentPage, totalPages);
  const paginatedList = useMemo(() => {
    if (!isOpen) return [];
    return filteredList.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);
  }, [isOpen, filteredList, safePage]);
