            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <div>
                {friends.length < 1 ? (
                  <button
                    disabled
                    className="px-3.5 py-1.5 text-xs font-bold rounded border border-zinc-700 bg-zinc-800/80 text-zinc-500 flex items-center gap-1.5 cursor-not-allowed"
                  >
                    <Gift className="w-4 h-4 text-zinc-600" />
                    <span>{isEn ? 'Need 1 Friend' : '未達成 (需 1 位好友)'}</span>
                  </button>
                ) : friendRewardClaimed ? (
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 text-xs font-black rounded border-2 border-black flex items-center gap-1.5 transition-all bg-amber-500 hover:bg-amber-400 text-black shadow-[inset_-2px_-2px_0_#b45309,inset_2px_2px_0_#fef08a] active:scale-95 cursor-pointer animate-pulse"
                  >
                    <Gift className="w-4 h-4" />
                    <span>{isEn ? 'Claim 100 Coins' : '領取 100 幣'}</span>
                  </button>
                )}
              </div>
            </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-[10px] text-zinc-400 font-bold">
                    {isEn ? 'Registered Account Name' : '註冊帳號名稱'}
                  </div>
                  <div className="font-black text-sm text-white font-minecraft tracking-wide">
                    {myUsername}
                  </div>
                </div>
              </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {onRemoveFriend && (
                        <button
                          onClick={() => {
                            sound.playClickSound();
                            onRemoveFriend(friend.code);
                          }}
                          title={isEn ? 'Remove Friend' : '移除好友'}
                          className="p-1 hover:bg-rose-950/70 text-zinc-500 hover:text-rose-400 border border-transparent hover:border-rose-700/60 rounded transition-all cursor-pointer"
                        >
